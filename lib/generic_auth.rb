class GenericAuth
  ...
  def valid_args?(args={})
    unless args.is_a? Hash;               return fail("missing_argument", "expected a :session hash"); end
    unless args[:oauth_client].present?;  return fail("missing_argument", "expected an :oauth_client argument"); end
    true
  end
